...
import { getStoredPushToken, registerForPushNotifications } from '../../services/notifications';

export function DevToolsOverlay() {
  if (!__DEV__) return null as any;

  const [open, setOpen] = useState(false);
  const [tab, setTab] = useState<'state' | 'logs' | 'errors' | 'queries' | 'notifs'>('state');
  ...
  const errors = useMemo(() => globalErrorHandler.getRecentErrors(50).reverse(), [tick]);
      ...
      <Modal visible={open} animationType="slide" onRequestClose={() => setOpen(false)}>
        <View style={styles.header}>
          ...
          <View style={styles.tabs}>
            {(['state', 'logs', 'errors', 'queries', 'notifs'] as const).map((t) => (
            ...
            <View>
              {errors.map((e, i) => (
                <View key={i} style={styles.errorBox}>
                  ...
                  {e.context && <Text style={styles.errorCtx}>{JSON.stringify(e.context)}</Text>}
                </View>
              ))}
            </View>
          )}
...
function AppStatePanel({ segments }: { segments: string[] }) {
  ...
  const state = {
    ...
    profile: userProfile?.id ? { id: userProfile.id, church_id: userProfile.church_id } : null,
...
const styles = StyleSheet.create({
  ...
  tab: { paddingVertical: 6, paddingHorizontal: 10, marginRight: 8, borderRadius: 6, backgroundColor: '#374151' },
  ...
  errorBox: { backgroundColor: '#fff0f0', borderColor: '#ffccd1', borderWidth: 1, borderRadius: 8, padding: 8, marginBottom: 8 },
  errorTitle: { color: '#b00020', fontWeight: '700' },
  errorMsg: { color: '#b00020' },
  errorCtx: { color: '#7f1d1d', fontFamily: 'monospace', fontSize: 11 },
  queryBox: { backgroundColor: '#fff', borderColor: '#e5e7eb', borderWidth: 1, borderRadius: 8, padding: 8, marginBottom: 8 },
...
function NotificationsPanel() {
  ...
  const fcmConfigured = Boolean((Constants as any)?.expoConfig?.android?.googleServicesFile || (Constants as any)?.android?.googleServicesFile);
  ...
  return (
    <View>
      <Text style={{ fontWeight: '700', marginBottom: 6 }}>Permissions: {permission}</Text>
      <Text style={{ marginBottom: 6 }}>Platform: {Platform.OS} | Device: {Device.isDevice ? 'physical' : 'emulator'}</Text>
      <Text style={{ marginBottom: 6 }}>App Ownership: {String(appOwnership || 'standalone')}</Text>
      <Text style={{ marginBottom: 6 }}>Expo Project ID: {String(projectId || '(none)')}</Text>
      <Text style={{ marginBottom: 6 }}>FCM Configured: {fcmConfigured ? 'yes' : 'no'}</Text>
      <Text selectable style={{ fontFamily: 'monospace', marginBottom: 8 }}>Expo Token: {token || '(none)'}</Text>
      <View style={{ flexDirection: 'row', gap: 8 }}>
        <TouchableOpacity style={[styles.tab, styles.tabActive]} onPress={requestPerms}>
          <Text style={styles.tabText}>Request Permissions</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.tab, styles.tabActive]} onPress={fetchToken}>
          <Text style={styles.tabText}>Get Token</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.tab, styles.tabActive]} onPress={scheduleLocal}>
          <Text style={styles.tabText}>Local Test</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.tab, styles.tabActive]} onPress={sendExpoTest}>
          <Text style={styles.tabText}>Send Expo Test</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.tab, styles.tabActive]} onPress={() => Notifications.getDevicePushTokenAsync().then((t) => console.log('[Notifications] Native device token:', t)).catch((e) => console.warn('Device token error:', e))}>
          <Text style={styles.tabText}>Native Token</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.tab, styles.tabActive]} onPress={() => Linking.openSettings().catch(() => {})}>
          <Text style={styles.tabText}>Open Settings</Text>
        </TouchableOpacity>
      </View>
      <Text style={{ marginTop: 10, color: '#6b7280' }}>
        Note: On Android, a dev client built with google-services.json is required for Expo push tokens. If using Expo Go or a build without FCM, token will be unavailable.
      </Text>
    </View>
  );
}
