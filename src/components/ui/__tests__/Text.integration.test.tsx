...
describe('Text Component Integration Tests', () => {
  describe('Theme Integration', () => {
    it('adapts colors correctly when theme changes from light to dark', () => {
      ...
      expect(textElement.props.style.color).toBe(lightTheme.colors.text.primary);
    ...
    it('maintains typography consistency across themes', () => {
      const { rerender } = render(
        <TestWrapperWithTheme theme="light">
          <Text variant="h1" testID="consistent-text">Consistent Typography</Text>
        </TestWrapperWithTheme>
      );
      ...
      expect(textElement.props.style.lineHeight).toBe(lightTypography.lineHeight);
      expect(textElement.props.style.fontWeight).toBe(lightTypography.fontWeight);
    });
    ...
    it('applies semantic colors correctly across themes', () => {
      ...
      semanticColors.forEach(color => {
        ...
        expect(textElement.props.style.color).toBe(lightTheme.colors[color][600]);
        ...
        expect(textElement.props.style.color).toBe(darkTheme.colors[color][600]);
      });
    });
  });
    ...
    it('maintains proper visual hierarchy across different variants', () => {
      render(
        <TestWrapperWithTheme>
          ...
          <Text variant="h2" testID="h2">Sub Heading</Text>
      ...
      expect(h1Element.props.style.fontSize).toBeGreaterThan(h2Element.props.style.fontSize);
      expect(h2Element.props.style.fontSize).toBeGreaterThan(bodyElement.props.style.fontSize);
      expect(bodyElement.props.style.fontSize).toBeGreaterThan(captionElement.props.style.fontSize);
      
      // Check font weight hierarchy (headings should be bolder)
      expect(parseInt(h1Element.props.style.fontWeight)).toBeGreaterThan(parseInt(bodyElement.props.style.fontWeight));
      expect(parseInt(h2Element.props.style.fontWeight)).toBeGreaterThan(parseInt(bodyElement.props.style.fontWeight));
    });
    ...
    it('applies consistent line height ratios for readability', () => {
      render(
        <TestWrapperWithTheme>
          ...
          <Text variant="bodyLarge" testID="body-large-text">Large Body Text</Text>
          <Text variant="bodySmall" testID="body-small-text">Small Body Text</Text>
        </TestWrapperWithTheme>
      );
      ...
      const bodyRatio = bodyElement.props.style.lineHeight / bodyElement.props.style.fontSize;
      const bodyLargeRatio = bodyLargeElement.props.style.lineHeight / bodyLargeElement.props.style.fontSize;
      const bodySmallRatio = bodySmallElement.props.style.lineHeight / bodySmallElement.props.style.fontSize;
  ...
  describe('Accessibility Integration', () => {
    it('provides proper accessibility roles for screen readers', () => {
      render(
        <TestWrapperWithTheme>
          <Text variant="h1" testID="heading">Main Heading</Text>
          <Text variant="body" testID="content">Content text</Text>
          <Text variant="caption" testID="caption">Caption</Text>
        </TestWrapperWithTheme>
      );
    ...
    it('provides accessibility labels for screen readers', () => {
      ...
      expect(textElement.props.accessibilityLabel).toBe('Important Information');
  ...
  describe('Style Composition Integration', () => {
    it('properly composes variant styles with custom overrides', () => {
      ...
      render(
        <TestWrapperWithTheme>
          <Text variant="h2" style={customStyle} testID="composed-text">Composed Text</Text>
        </TestWrapperWithTheme>
      );
    ...
    it('allows style overrides to take precedence over variant styles', () => {
      ...
      render(
        <TestWrapperWithTheme>
          <Text variant="h1" style={overrideStyle} testID="override-text">Override Text</Text>
        </TestWrapperWithTheme>
      );
  ...
  describe('Performance Integration', () => {
    ...
    it('handles style recalculation efficiently', () => {
      const { rerender } = render(
        <TestWrapperWithTheme>
          <Text variant="body" color="primary" testID="perf-text">Performance Test</Text>
        </TestWrapperWithTheme>
      );
      ...
      for (let i = 0; i < 50; i++) {
        ...
        rerender(
          <TestWrapperWithTheme>
            <Text variant={variant} color={color} testID="perf-text">Performance Test</Text>
          </TestWrapperWithTheme>
        );
      }
