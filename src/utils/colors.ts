 ...
 */
export const meetsWCAGAALarge = (foreground: string, background: string): boolean => {
 ...
 */
export const meetsWCAGAAALarge = (foreground: string, background: string): boolean => {
...
export const findAccessibleColor = (
  ...
  const requiredRatio = standard === 'AAA' ? WCAG_STANDARDS.AAA_NORMAL : WCAG_STANDARDS.AA_NORMAL;
...
export const generateThemeColors = (
  ...
  return Object.entries(baseColors).reduce((acc, [key, color]) => {
    ...
    return acc;
  }, {} as Record<string, ColorVariants & { semantic: ReturnType<typeof generateSemanticVariants> }>);
};
 ...
 */
export const getAnalogousColors = (color: string, angle: number = 30): [string, string] => {
  const rgb = hexToRgb(color);
