...
export const migrationHelpers = {
  ...
  generateMigrationDocs(
    ...
    if (mappings.length > 0) {
      ...
      lines.push('');
      lines.push('| Legacy Property | Legacy Value | Theme Token | Confidence | Needs Review |');
      lines.push('|----------------|--------------|-------------|------------|--------------|');
    ...
    lines.push('```tsx');
    lines.push(`import { ${componentName} } from './components/${componentName}';`);
    lines.push(`import { ThemeProvider } from './theme/provider/ThemeProvider';`);
    lines.push('');
    ...
    if (config.addCompatibilityLayer) {
      ...
      lines.push('');
      lines.push('This component includes a compatibility layer that allows it to work with both legacy and theme-aware styles.');
      lines.push('The compatibility layer will show deprecation warnings in development mode.');
      lines.push('');
    }
    ...
    if (reviewItems.length > 0) {
      ...
      reviewItems.forEach(item => {
        lines.push(`- **${item.legacyProperty}**: ${item.legacyValue} → ${item.themeTokenPath} (${Math.round(item.confidence * 100)}% confidence)`);
      });
      lines.push('');
    }
   ...
   */
  generateMigrationTests(componentName: string, mappings: LegacyStyleMapping[]): string {
    const lines: string[] = [];
    ...
    lines.push('  const renderWithTheme = (component: React.ReactElement) => {');
    ...
    lines.push(`    const { getByTestId } = renderWithTheme(<${componentName} testID="${componentName.toLowerCase()}" />);`);
    lines.push(`    const component = getByTestId('${componentName.toLowerCase()}');`);
    ...
    if (mappings.length > 0) {
      ...
      lines.push('    // Add specific style assertions based on your component');
  ...
  validateMigration(
    ...
    if (migratedCode.includes('StyleSheet.create') && !migratedCode.includes('theme')) {
    ...
    highConfidenceMappings.forEach(mapping => {
      if (!migratedCode.includes(mapping.themeTokenPath.replace('theme.', ''))) {
        issues.push(`High-confidence mapping not applied: ${mapping.legacyProperty} → ${mapping.themeTokenPath}`);
      }
    });
   ...
   */
  generateMigrationChecklist(componentName: string, mappings: LegacyStyleMapping[]): string[] {
    ...
    if (mappings.length > 0) {
      ...
      const reviewItems = mappings.filter(m => m.needsReview);
      if (reviewItems.length > 0) {
        checklist.push(`⚠️ Review ${reviewItems.length} low-confidence mappings`);
      }
    }
  ...
  extractComponentMetadata(code: string): {
    ...
    const importStatements = Array.from(code.matchAll(importRegex)).map(match => match[0]);
   ...
   */
  generateRollbackInstructions(componentName: string, backupPath?: string): string {
    const lines: string[] = [];
