...
export async function runBasicBatchMigration(): Promise<MigrationReport> {
  ...
  if (dryRunReport.failedMigrations === 0) {
    ...
    console.log(
      'Dry run had failures, please review before running actual migration'
    );
    return dryRunReport;
  }
}
...
export async function runIncrementalMigration(): Promise<void> {
  ...
  console.log(
    `UI components: ${uiReport.successfulMigrations}/${uiReport.totalComponents} migrated`
  );
  ...
  console.log(
    `Form components: ${formReport.successfulMigrations}/${formReport.totalComponents} migrated`
  );
  ...
  console.log(
    `Navigation components: ${navReport.successfulMigrations}/${navReport.totalComponents} migrated`
  );

  // Step 4: Migrate remaining components with safe settings
  console.log('Step 4: Migrating remaining components...');
  const safeScript = migrationPresets
    .safeMigration()
  ...
  console.log(
    `Remaining components: ${remainingReport.successfulMigrations}/${remainingReport.totalComponents} migrated`
  );
...
export async function runSafeMigrationWithRollback(): Promise<MigrationReport> {
  ...
  try {
    ...
    if (report.failedMigrations > 0) {
      console.warn(
        `Migration completed with ${report.failedMigrations} failures`
      );
      console.log(
        'Failed components can be rolled back using their backup files'
      );
      ...
      Object.entries(report.componentResults).forEach(([filePath, result]) => {
        if (!result.success && result.backupPath) {
          console.log(
            `Rollback ${filePath}: cp ${result.backupPath} ${filePath}`
          );
        }
      });
    }
...
export async function runAggressiveMigration(): Promise<MigrationReport> {
  ...
  console.warn(
    'This migration removes legacy style support - ensure thorough testing!'
  );
...
export async function runMigrationWithValidation(): Promise<MigrationReport> {
  ...
  Object.entries(report.componentResults).forEach(([filePath, result]) => {
    if (result.success) {
      ...
      if (
        result.migratedCode.includes('StyleSheet.create') &&
  ...
  if (validationErrors > 0) {
    console.warn(
      `Validation found ${validationErrors} issues that need attention`
    );
  } else {
...
export function generateMigrationReport(report: MigrationReport): string {
  ...
  lines.push(
    `**Success Rate**: ${Math.round((report.successfulMigrations / report.totalComponents) * 100)}%`
  );
  lines.push('');
