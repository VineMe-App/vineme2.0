...
import { permissionService, type UserRole, type Permission, type PermissionCheck } from '../services/permissions';

interface UsePermissionsReturn {
  hasRole: (role: UserRole) => Promise<boolean>;
  hasAnyRole: (roles: UserRole[]) => Promise<boolean>;
  hasPermission: (permission: Permission, resourceId?: string) => Promise<PermissionCheck>;
  canAccessChurchData: (churchId: string) => Promise<PermissionCheck>;
  canModifyResource: (resourceType: 'user' | 'group' | 'event', resourceId: string, ownerId?: string) => Promise<PermissionCheck>;
  canManageGroupMembership: (groupId: string, targetUserId?: string) => Promise<PermissionCheck>;
...
export const usePermissions = (): UsePermissionsReturn => {
  ...
  const hasAnyRole = useCallback(async (roles: UserRole[]): Promise<boolean> => {
    setIsLoading(true);
    setError(null);
  ...
  const hasPermission = useCallback(async (permission: Permission, resourceId?: string): Promise<PermissionCheck> => {
    setIsLoading(true);
    setError(null);
  ...
  const canAccessChurchData = useCallback(async (churchId: string): Promise<PermissionCheck> => {
    setIsLoading(true);
    setError(null);
  ...
  const canModifyResource = useCallback(async (
    ...
    try {
      const result = await permissionService.canModifyResource(resourceType, resourceId, ownerId);
      return result;
    } catch (error) {
      handleError(error, 'check resource modification');
      return { hasPermission: false, reason: 'Resource modification check failed' };
    } finally {
      setIsLoading(false);
    }
  }, []);

  const canManageGroupMembership = useCallback(async (
    groupId: string,
    targetUserId?: string
  ): Promise<PermissionCheck> => {
    setIsLoading(true);
    ...
    try {
      const result = await permissionService.canManageGroupMembership(groupId, targetUserId);
      return result;
    } catch (error) {
      handleError(error, 'check group membership management');
      return { hasPermission: false, reason: 'Group membership check failed' };
    } finally {
      setIsLoading(false);
    }
  }, []);
 ...
 */
export const usePermissionCheck = (permission: Permission, resourceId?: string): UsePermissionCheckReturn => {
