...
describe('adminPagination', () => {
  ...
  describe('mergePaginatedData', () => {
    it('should merge unique data correctly', () => {
      ...
      const merged = mergePaginatedData(existing, newData, (item) => item.id);
    ...
    it('should handle empty existing data', () => {
      ...
      const merged = mergePaginatedData([], newData, (item) => item.id);
  ...
  describe('ADMIN_PAGINATION_DEFAULTS', () => {
    it('should have reasonable default values', () => {
      ...
      expect(
        ADMIN_PAGINATION_DEFAULTS.INFINITE_SCROLL_THRESHOLD
      ).toBeGreaterThan(0);
    });
  });
});
